...
Goal: summarize the current session, document durable memory to the repo-local wiki with a cheap/smaller-model worker, and write `session_handoff.md` at the workspace root.

Work only inside the current working folder for the active project. If this prompt is being used to bootstrap a new folder, that folder is the workspace.
...
4. Create `session_handoff.md` at the workspace root using the structure from `prompts/summarize-for-handoff.md`. Keep it under 2000 estimated tokens. Preserve exact paths, commands, errors, decisions, blockers, commits, active unfinished subagents, and next-session requirements. Exclude transcript noise, raw logs, broad wiki pages, docs-only discoveries, and large plans that should be rebuilt in fresh context.
