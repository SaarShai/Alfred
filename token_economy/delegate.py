...
def classify(task: str, registry: dict[str, list[str]] | None = None) -> Route:
    ...
    if CHIEF_OF_STAFF_RE.search(task):
        chief_parallel = parallel or bool(re.search(r"\b(briefing|weekly|research|sources?|follow[- ]ups?|extract|subagents?|approved)\b", text))
        return Route(
            "medium",
            "medium",
            models["medium"][0],
            ...
            0.84,
            chief_parallel,
            "use user operating profile and approved-source registry; send compact task packet only; draft then ask; final synthesis stays with main agent",
            "chief-of-staff briefing, source, preference, or follow-up task",
        )
...
def delegation_plan(task: str, registry: dict[str, list[str]] | None = None) -> dict[str, Any]:
    ...
    brief = {
        ...
        "output": "compact result packet: outcome, sources, confidence, verification, risks, follow_ups; add changed files for code/wiki edits",
    ...
    return {
        ...
        "result_contract": ["outcome", "sources", "confidence", "verification", "risks", "follow_ups", "changed files"],
...
def personal_assistant_packet(task: str, registry: dict[str, list[str]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "result_contract": {
            ...
            "required": ["answer_or_outcome", "sources_or_evidence", "confidence", "verification", "risks", "follow_ups"],
...
def personal_assistant_directive(packet: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "Expected result: answer_or_outcome, sources_or_evidence, confidence, verification, risks, follow_ups; add changed_files/tests/exact_errors for code.",
    ]
    return "\n".join(lines)
