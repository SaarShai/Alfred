...
class WikiStore:
    ...
    def index(self) -> dict[str, Any]:
        ...
        pages = self.pages()
        self.state_dir.mkdir(exist_ok=True)
        lock_path = self.state_dir / "wiki.index.lock"
        ...
        for page in ordered:
            ...
            if page.id == "INSTALL":
                continue
            if page.id in bundled_framework_pages and page.id not in l1_always_include:
                continue
            parts = set(Path(page.id).parts)
            if parts & l1_support_dirs and page.id not in l1_always_include:
                continue
    ...
    def _ensure_db(self) -> None:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("SELECT 1 FROM docs LIMIT 1").fetchone()
    ...
    def _rank_pages(self, query: str) -> list[tuple[Page, float, list[str]]]:
        ...
        raw_requested = bool(RAW_CONTEXT_RE.search(query))
        prompt_requested = bool(PROMPT_CONTEXT_RE.search(query))
        pages = self.pages()
        ...
        for page in pages:
            ...
            if page.id.startswith("prompts/") and not prompt_requested:
                score -= 3.0
                reasons.append("prompt-downranked")
    ...
    def context(self, task: str, max_pages: int = 5, max_tokens: int = 4000, k: int = 12) -> dict[str, Any]:
        ...
        self._ensure_db()
        raw_requested = bool(RAW_CONTEXT_RE.search(task))
