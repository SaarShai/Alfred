...
from .promptkit import draft_prompt, list_entries as list_prompt_entries
...
def cmd_chief(args: argparse.Namespace) -> int:
    cfg = load_config(args.repo)
    if args.chief_cmd == "readiness":
        print_json(chief_readiness(cfg.repo_root))
    elif args.chief_cmd == "onboarding":
...
def cmd_prompt(args: argparse.Namespace) -> int:
    ...
    if args.prompt_cmd == "list":
        print_json(list_prompt_entries(cfg.repo_root, args.kind))
    elif args.prompt_cmd == "draft":
        packet = draft_prompt(
            cfg.repo_root,
            args.requirement,
            template_slug=args.template,
            snippet_slugs=args.section,
            var_pairs=args.var,
        )
        if args.format == "json":
            print_json(packet)
        else:
            print(packet["rendered_prompt"])
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    chief = sub.add_parser("chief", help="Chief-of-staff readiness, onboarding, and briefing helpers")
    ...
    prompt = sub.add_parser("prompt", help="Compose reusable prompt templates and snippets")
    promptsub = prompt.add_subparsers(dest="prompt_cmd", required=True)
    pl = promptsub.add_parser("list")
    pl.add_argument("--kind", choices=["template", "templates", "snippet", "snippets"], default=None)
    pl.set_defaults(func=cmd_prompt)
    pd = promptsub.add_parser("draft")
    pd.add_argument("requirement")
    pd.add_argument("--template", help="Template slug from prompts/library/templates")
    pd.add_argument("--section", action="append", help="Snippet slug from prompts/library/snippets; repeatable")
    pd.add_argument("--var", action="append", help="Template variable as key=value; repeatable")
    pd.add_argument("--format", choices=["prompt", "json"], default="prompt")
    pd.set_defaults(func=cmd_prompt)
