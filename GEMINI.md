...
### Durable memory store (`L1_index.md` + `./te wiki`)

This repo carries a curated knowledge store — the *why/decision/
...
`L1_index.md` first, then `./te wiki search "<q>"` → `./te wiki timeline <id>`
→ `./te wiki fetch <id>`. Maintained by `wiki-memory` (write) and `wiki-refresh`
(reconcile vs code).
